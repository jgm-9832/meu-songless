...
// ==========================================
// 2. INICIALIZANDO O SPOTIFY VIRTUAL (SDK)
// ==========================================
...
async function iniciarJogo() {
    ...
    if (!tokenDeAcesso) {
        ...
        msgArea.innerText = "✅ Autenticado! Ligando o motor do Player Premium...";
...
// ==========================================
// 3. PUXANDO SUAS MÚSICAS (SEM CLONES) E SORTEIO
// ==========================================
async function carregarMusicas() {
    try {
        ...
        while (url && paginasBuscadas < limiteDePaginas) {
            msgArea.innerText = `Lendo seu cofre Premium... (Página ${paginasBuscadas + 1} de ${limiteDePaginas})`;
            ...
            const todasAsMusicas = dados.items
                .map(item => item.track)
        ...
        if (playlistReal.length > 0) {
            msgArea.innerText = `🔥 ${playlistReal.length} músicas únicas carregadas! Aperte Play.`;
            sortearMusica();
        } else {
            msgArea.innerText = "Nenhuma música encontrada na sua biblioteca.";
        }
    } catch (erro) {
        console.error(erro);
...
// ==========================================
// 4. LÓGICA DO JOGO, MODO VIBE E MINI-PLAYER
// ==========================================
...
// 🌟 Lógica do Botão de Pausar/Tocar o Modo Vibe
...
function atualizarStreak(ganhou) {
    ...
    setTimeout(() => {
        streakDisplay.style.color = '#ff9800';
    }, 1500);
}
...
function mostrarCapaDaMusica(acertou) {
    ...
    // 🌟 Configura os botões do Mini-Player
...
btnPlay.addEventListener('click', async () => {
    if (!tokenDeAcesso || !deviceId) {
        alert("Aguarde o Player Premium se conectar!");
        return;
    }
